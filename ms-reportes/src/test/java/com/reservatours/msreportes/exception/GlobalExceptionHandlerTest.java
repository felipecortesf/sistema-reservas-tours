package com.reservatours.msreportes.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleValidationErrors_retorna400ConCamposInvalidos() {
        ...
        FieldError fieldError = new FieldError("reporteDto", "tipo", "no debe estar vacio");
        ...
        assertEquals("no debe estar vacio", response.getBody().get("tipo"));
    }

    @Test
    void handleRuntimeException_retorna500() {
        RuntimeException ex = new RuntimeException("Error inesperado al generar reporte");

        ResponseEntity<Map<String, String>> response = handler.handleRuntimeException(ex);

        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, response.getStatusCode());
        assertEquals("Error inesperado al generar reporte", response.getBody().get("error"));
    }
