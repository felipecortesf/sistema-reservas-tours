package com.reservatours.mscomunicacionagencia.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleValidationErrors_retorna400ConCamposInvalidos() {
        ...
        FieldError fieldError = new FieldError("mensajeDto", "contenido", "no debe estar vacio");
    ...
    void handleRuntimeException_retorna500() {
        RuntimeException ex = new RuntimeException("Error inesperado al guardar mensaje");

        ResponseEntity<Map<String, String>> response = handler.handleRuntimeException(ex);

        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, response.getStatusCode());
        assertEquals("Error inesperado al guardar mensaje", response.getBody().get("error"));
    }
